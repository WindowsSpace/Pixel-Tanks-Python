...
class Tank(pygame.sprite.Sprite):
    ...
    def can_move_to(self, gx, gy, collision_group=None) -> bool:
        ...
        if collision_group:
            temp_hitbox = pygame.Rect(0, 0, TankHitbox, TankHitbox)
            temp_hitbox.center = grid_to_pixel_center(gx, gy)
            for sprite in collision_group:
                if sprite is not self and temp_hitbox.colliderect(sprite.hitbox): return False
        return True
...
class PlayerTank(Tank):
    ...
    def handle_keydown(self, direction) -> None:
...
class EnemyTank(Tank):
    ...
    def update_ai(self, dt_ms, player, collision_group, bullets_group, all_sprites_group, speed_mult=1.0) -> None:
        # Применяем множитель скорости игры к таймерам (чем выше уровень, тем меньше таймеры)
        ...
        if self.move_timer <= 0:
            if random.random() < 0.6: # Чуть агрессивнее
